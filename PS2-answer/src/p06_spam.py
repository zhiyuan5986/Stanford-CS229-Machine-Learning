...
def fit_naive_bayes_model(matrix, labels):
    ...
    # *** START CODE HERE ***
    print(np.sum(matrix[labels == 1],axis=0).shape)
    print(labels.shape)
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    # print(np.exp(np.sum(matrix @ np.log(model[1]))))
    # p_y1_x = 1 / (1+np.exp(np.sum(np.log(model[1])))*model[2]/np.exp(np.sum(np.log(model[0])))/(1-model[2]))
    # print(p_y1_x)
    # *** END CODE HERE ***
