...
# state is actually beta and x_j, j=1,2,\dots,i
...
def initial_state():
    ...
    # *** START CODE HERE ***
    return [[],[]] # first for beta, second for x_j
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    for j in range(len(state[0])):
        pred += state[0][j]*kernel(state[1][j],x_i)
    return pred >=0
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    print(pred)
    state[0].append(learning_rate*(y_i-pred))
    state[1].append(np.array(x_i))
    # *** END CODE HERE ***
