...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    pi = mdp_data['transition_probs'][state].T @ mdp_data['value']
    return np.argmax(pi) if pi[0] != pi[1] else np.random.randint(2)
    # *** END CODE HERE ***
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    mdp_data['reward_counts'][new_state][0] += np.abs(reward)
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # mdp_data['transition_probs'][index][:,:,0] = 1 / ( 1+ mdp_data['transition_counts'][index][:,:,1] / mdp_data['transition_counts'][index][:,:,0])
    # mdp_data['transition_probs'][index][:,:,1] = 1-mdp_data['transition_probs'][index][:,:,0] 
    # mdp_data['reward'][index] = -mdp_data['reward_counts'][index][:,0] / mdp_data['reward_counts'][index][:,1]
    # mdp_data['transition_probs'][:,index,0] = 1 / ( 1+ mdp_data['transition_counts'][:,index,1] / mdp_data['transition_counts'][:,index,0])
    # mdp_data['transition_probs'][:,index,1] = 1-mdp_data['transition_probs'][:,index,0] 

    # refer to https://github.com/huyfam/cs229-solutions-2020
    transition_counts = mdp_data['transition_counts']
    num_states = mdp_data['num_states']
    reward_counts = mdp_data['reward_counts']
    state_action_counts = np.sum(mdp_data['transition_counts'], axis=1, keepdims=True)
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        mdp_data['value'] = mdp_data['reward'] + gamma*np.max(np.vstack((mdp_data['transition_probs'][:,:,0]@value_old,mdp_data['transition_probs'][:,:,1]@value_old)),axis=0)
        if np.max(np.abs(mdp_data['value']-value_old)) < tolerance:
            break
    return iters==1
    # iters = 0
...
def plot_trial(seed,mdp_data):
    ...
    while True:
        ...
        state = new_state
    # create gif file
    with imageio.get_writer(f'../output/simulation{seed}.gif', mode='I') as writer:
        for filename in files:
            image = imageio.imread(f'../output/frames/{filename}')
            writer.append_data(image)
    # remove redundancy
    shutil.rmtree("../output/frames")

def main(plot=True,plot_simulation=False):
    ...
    if plot_simulation:
        ...
        plot_trial(seed,mdp_data)
    return np.array(time_steps_to_failure)
    
if __name__ == '__main__':
    main(plot_simulation=False)
